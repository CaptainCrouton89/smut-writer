  ...
  return (
    <div className="w-full max-w-5xl mx-auto relative">
      ...
      <div className="absolute inset-0 pointer-events-none rounded-3xl overflow-hidden">
        <svg className="absolute inset-0 w-full h-full opacity-25" preserveAspectRatio="none" viewBox="0 0 100 100" fill="none">
          <defs>
            <linearGradient id="storyGradient" x1="0%" y1="0%" x2="100%" y2="100%">
              ...
              <stop offset="100%" stopColor="currentColor" stopOpacity="0.06" className="text-accent" />
              <animateTransform
      ...
      <div className="relative p-6 sm:p-8 md:p-12 lg:p-16 xl:p-20">
        ...
        <div className="text-center space-y-8 md:space-y-12">
          <div ref={storyTextRef} className="max-w-4xl mx-auto px-2 transition-all duration-500 ease-in-out">
            {showThinking ? (
              <div className="text-lg sm:text-xl md:text-2xl lg:text-3xl text-foreground/60 leading-relaxed font-light italic min-h-[2em] flex items-center justify-center animate-fade-in">
                ...
                <span className="ml-1 animate-bounce" style={{ animationDelay: '0.1s' }}>.</span>
                <span className="ml-1 animate-bounce" style={{ animationDelay: '0.2s' }}>.</span>
              </div>
            ) : (
          ...
          {currentStory.choices && (
            <div className={`max-w-2xl mx-auto px-2 transition-all duration-1000 ease-out overflow-hidden ${
              !showThinking && showChoices ? 'h-auto' : 'h-0'
            }`}>
              <div className={`space-y-4 sm:space-y-6 transition-all duration-700 ease-in-out ${
                !showThinking && showChoices 
                  ? 'opacity-100 translate-y-0' 
                  : 'opacity-0 translate-y-8 pointer-events-none'
              }`}>
                <div className={`text-center transition-all duration-500 ${
                  !showThinking && showChoices ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                }`}>
                  <p className="text-base sm:text-lg text-accent font-medium mb-6 sm:mb-8">What do you do next?</p>
                </div>
                ...
                {currentStory.choices.map((choice, index) => (
                  <Button
                    ...
                    style={{ 
                      transitionDelay: !showThinking && showChoices ? `${index * 0.15}s` : '0s'
                    }}
                    onClick={() => handleChoice(choice.next)}
                  >
                    <span className="text-accent mr-2 sm:mr-4 flex-shrink-0 transition-transform duration-200 group-hover:translate-x-1">→</span>
                    <span className="text-left leading-relaxed break-words">{choice.text}</span>
                  </Button>
                ))}
              </div>
            </div>
          ...
          {showWaitlist && !isTyping && (
            <div className="text-center space-y-6 sm:space-y-8 pt-6 sm:pt-8 border-t border-primary/20 max-w-2xl mx-auto px-2 animate-fade-in">
              <div className="space-y-3 sm:space-y-4 animate-slide-up">
                ...
                <h3 className="text-xl sm:text-2xl font-bold text-primary animate-fade-in" style={{ animationDelay: '0.2s' }}>Want to explore further?</h3>
                <p className="text-base sm:text-lg text-muted-foreground animate-fade-in" style={{ animationDelay: '0.3s' }}>
